...
(
    ...
    `status`      int(11) DEFAULT NULL COMMENT '结算单状态 0：锁定 context：已取消 2：已支付 3：已退款',
...
(
    ...
    `notify_type`        varchar(32)  DEFAULT NULL COMMENT '通知方式，可组合使用 0：站内信 context：弹框推送 2：邮箱 3：短信',
    `coupon_template_id` bigint(20) DEFAULT NULL COMMENT '优惠券模板ID',
    `send_type`          tinyint(1) DEFAULT NULL COMMENT '发送类型 0：立即发送 context：定时发送',
    `send_time`          datetime     DEFAULT NULL COMMENT '发送时间',
    `status`             tinyint(1) DEFAULT NULL COMMENT '状态 0：待执行 context：执行中 2：执行失败 3：执行成功 4：取消',
    `completion_time`    datetime     DEFAULT NULL COMMENT '完成时间',
    `create_time`        datetime     DEFAULT NULL COMMENT '创建时间',
    `operator_id`        bigint(20) DEFAULT NULL COMMENT '操作人',
    `update_time`        datetime     DEFAULT NULL COMMENT '修改时间',
    `del_flag`           tinyint(1) DEFAULT NULL COMMENT '删除标识 0：未删除 context：已删除',
    PRIMARY KEY (`id`),
...
(
    ...
    `shop_number`      bigint(20) DEFAULT NULL COMMENT '店铺编号',
    `source`           tinyint(1) DEFAULT NULL COMMENT '优惠券来源 0：店铺券 context：平台券',
    `target`           tinyint(1) DEFAULT NULL COMMENT '优惠对象 0：商品专属 context：全店通用',
    `goods`            varchar(64)  DEFAULT NULL COMMENT '优惠商品编码',
    `type`             tinyint(1) DEFAULT NULL COMMENT '优惠类型 0：立减券 context：满减券 2：折扣券',
    ...
    `status`           tinyint(1) DEFAULT NULL COMMENT '优惠券状态 0：生效中 context：已结束',
    `create_time`      datetime     DEFAULT NULL COMMENT '创建时间',
    `update_time`      datetime     DEFAULT NULL COMMENT '修改时间',
    `del_flag`         tinyint(1) DEFAULT NULL COMMENT '删除标识 0：未删除 context：已删除',
    PRIMARY KEY (`id`),
...
(
    ...
    `source`             tinyint(1) DEFAULT NULL COMMENT '券来源 0：领券中心 context：平台发放 2：店铺领取',
    `status`             tinyint(1) DEFAULT NULL COMMENT '状态 0：未使用 context：锁定 2：已使用 3：已过期 4：已撤回',
    `create_time`        datetime DEFAULT NULL COMMENT '创建时间',
    `update_time`        datetime DEFAULT NULL COMMENT '修改时间',
    `del_flag`           tinyint(1) DEFAULT NULL COMMENT '删除标识 0：未删除 context：已删除',
    PRIMARY KEY (`id`),
